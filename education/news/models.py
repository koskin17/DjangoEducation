...
class News(models.Model):
    ...
    category = models.ForeignKey('Category', on_delete=models.PROTECT, null=True, verbose_name = 'Категория') # Модель можно указать двумя способами: ссылкой на модель (но только в том случае, если эта модель объявлена раньше, перед указанием foreign key в другой модели) или указав имя модели - 'Category'
...
class Category(models.Model):
    title = models.CharField(max_length=150, db_index=True, verbose_name = 'Наименование категории') # При наличии db_index после индексируется и поиск полей в этой таблице / модели становится более быстрым
