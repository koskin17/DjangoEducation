...
class News(models.Model):
    ...
    def get_absolute_url(self): # Благодаря этому методу при использовании класса CreatedView после создания новости происходит автоматический редирект на новую новость. Происходит это из-за того, что этот метод назван именно так и django именно такое название ожидает в некоторых местах. И именно в момент добавления объекта через CreatedView django смотрит, есть ли у модели News (к этой модели подвязана форма) метод get_absolute_url и если есть, то он его использует для постоения ссылки, на которую нужно сделать редирект. Теперь, при классе CreateNews от класса CreateView, если этого метода не будет, то будет ошибка о том, что нет url, на который надо сделать редирект
